...
- `scripts/`: offline generation only. `generate-static-entities.ts` (the top-N-per-type static precompute) and `benchmark.ts`; `lib/` holds the SSH/DuckDB query layer (`duckdb-ssh.ts`, `parquet-store.ts`, `cnpj-index.ts`) used only by these scripts, never imported by `src/`.
...
`bun run generate:static` (flags: `--top=50`, `--per-dataset-limit=15`, `--concurrency=6`, `--query-timeout-ms=60000`, `--max-distinct=1000000`, `--force`) is the **only** thing in this repo that talks to beelink. Two passes:
